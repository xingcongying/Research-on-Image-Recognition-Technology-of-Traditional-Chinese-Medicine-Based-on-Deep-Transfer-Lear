...
def evaluate(val_loader,model,criterion,epoch):
    ...
    top1 = AverageMeter()
    #progress bar
    val_progressor = ProgressBar(mode="Val  ",epoch=epoch,total_epoch=config.epochs,model_name=config.model_name,total=len(val_loader),learning_rate=0.0)
...
def main():
    ...
    for epoch in range(start_epoch,config.epochs):
        ...
        for iter,(input,target) in enumerate(train_dataloader):
            ...
            train_progressor.learning_rate = (get_learning_rate(optimizer)*100)/100
            #backward
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
